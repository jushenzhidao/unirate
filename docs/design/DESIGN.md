...
类型                [ 速率 rate | 并发 concurrency ]  分段控件
  ...
  时间窗口          [ 5m ]  预设 1s/1m/5m/1h/1d/1w/2w + 自定义
                    下拉用原生 optgroup 分两族：滚动窗口 1s/1m/5m/1h、自然对齐 1d/1w/2w
                    自然对齐族附「零点位置取决于部署的 TZ_OFFSET_SECONDS，改动需重启」
                    界面禁止显示具体时区数值或推算时刻（前端无法获取运行时真值）
