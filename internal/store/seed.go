...
// 种子数据加载。
//
// 为什么需要它：MySQL 官方镜像用 /docker-entrypoint-initdb.d 提供了这个
// 能力，e2e 的 demo 业务域靠它注入。切到 SQLite 后数据库由 gateway 进程
// 自己创建，那条注入路径消失了 —— 不补齐，e2e 会因为「找不到 demo 业务域」
// 大面积失败，而失败原因看起来像限流逻辑出错，极难定位。
//
// 设计约束：
...
//
// 排序语义与 MySQL 镜像一致（01- 建表、02- 数据），保证迁移前后
// 种子文件的编号约定不用改。
//
